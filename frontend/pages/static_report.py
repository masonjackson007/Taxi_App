...
# Assumes this script is in 'frontend/pages/' and images are in 'frontend/assets/report/'
# Go up one level from 'pages' to 'frontend', then down into 'assets/report'
# Alternatively, if Streamlit serves from the project root, 'frontend/assets/report/' might work.
# Using relative paths from the script is often more robust within Streamlit pages.

# Let's try a simpler relative path assuming Streamlit context handles it well
# If 'pages' and 'assets' are both folders inside 'frontend', this should work:
IMAGE_DIR = Path("assets/")
# If that doesn't work, try specifying relative to the script's parent directory:
# SCRIPT_DIR = Path(__file__).parent
# IMAGE_DIR = SCRIPT_DIR.parent / "assets" / "report"
